...
namespace laal
{
	MovieReader::MovieReader(const std::string& filename_, unsigned int width_, unsigned int height_) :
		fc(NULL),
		width(width_),
		height(height_)
	{
		...
		const std::string filename = filename_ + "." + fmtext;
	...
	bool MovieReader::GetFrame(std::vector<uint8_t>& pixels)
	{
		while (1)
		{
			AVPacket pkt;
